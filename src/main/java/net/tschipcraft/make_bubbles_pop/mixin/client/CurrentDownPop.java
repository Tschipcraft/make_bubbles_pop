...
/*public abstract class CurrentDownPop extends TextureSheetParticle {
	...
	protected CurrentDownPop(ClientLevel level, double x, double y, double z) {
		super(level, x, y, z);
	}
*///?} >=1.21.9 {
public abstract class CurrentDownPop extends SingleQuadParticle {

	protected CurrentDownPop(ClientLevel level, double x, double y, double z, TextureAtlasSprite sprite) {
...
//?}
	...
	void makeBubblesPop$init(ClientLevel level, double x, double y, double z, CallbackInfo ci) {
	*///?} >=1.21.9 {
	@Inject(method = "<init>", at = @At(value = "TAIL"))
	void makeBubblesPop$init(ClientLevel level, double x, double y, double z, TextureAtlasSprite sprite, CallbackInfo ci) {
	//?}
	...
	private void makeBubblesPop$injectFloorCollision(CallbackInfo ci) {
		if (!this.level.isWaterAt(BubbleUtil.blockPos(this.x, this.y - this.quadSize * 1.5D, this.z))) {
			this.yd = 0.01D;
