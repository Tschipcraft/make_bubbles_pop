...
public class BubbleUtil {
    ...
    public static void popBubble(Level world, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
        if (!MakeBubblesPop.MIDNIGHTLIB_INSTALLED || MakeBubblesPopConfig.POP_PARTICLE_ENABLED) {
            ...
            world.playLocalSound(x, y, z, SoundEvents.BUBBLE_COLUMN_BUBBLE_POP, SoundSource.AMBIENT, (MakeBubblesPop.MIDNIGHTLIB_INSTALLED ? MakeBubblesPopConfig.BUBBLE_POP_VOLUME : 0.1F) - (world.getRandom().nextFloat() * 0.1F), 0.85F + (world.getRandom().nextFloat() * 0.3F), false);
        }
    }
     ...
     */
    public static void tintBubble(Level world, double x, double y, double z, SingleQuadParticle particle) {
        if (!MakeBubblesPop.MIDNIGHTLIB_INSTALLED || MakeBubblesPopConfig.BIOME_COLORS_ENABLED) {
            // Tint bubbles based on the water color
            int waterColor = BiomeColors.getAverageWaterColor((BlockAndTintGetter) world, BlockPos.containing(x, y, z));
